...
class CameraObject:
    ...
    # New generate streame for lane detector
    def generate_stream(self):
        last_resolution = None

        while True:
            if self.capturing_still:
                frame_bytes = self.placeholder_frame
            else:
                with self.output.condition:
                    self.output.condition.wait()
                    frame_bytes = self.output.read_frame()
            ...
            try:
                ...
                if img is not None:
                    processed_img, angle = self.lane_detector.detect_lanes(img)
                    
                    _, jpeg_frame = cv2.imencode('.jpg', processed_img)
                    final_frame = jpeg_frame.tobytes()
                else:
